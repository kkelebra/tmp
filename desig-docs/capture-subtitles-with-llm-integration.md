...
// Seleccionar el contenedor de subtítulos
const subtitleContainer = document.querySelector('.subtitle-container-class'); // Cambiar según la plataforma

// Almacenar el último subtítulo procesado
let lastSubtitle = '';
let lastSpeaker = '';

// Observar cambios en el contenedor de subtítulos
const observer = new MutationObserver(mutations => {
  mutations.forEach(mutation => {
    const newSubtitle = subtitleContainer.innerText.trim();

    if (newSubtitle && newSubtitle !== lastSubtitle) {
      // Detectar cambio de hablante
      const [currentSpeaker, ...content] = newSubtitle.split(':');
      const subtitleContent = content.join(':').trim();

      if (currentSpeaker !== lastSpeaker) {
        console.log('Cambio de hablante detectado:', currentSpeaker);

        // Enviar subtítulos al servidor WebSocket
...
#### **4.2 Reconocimiento de Turnos de Habla**
El reconocimiento de turnos de habla se basa en la detección de cambios en el prefijo del subtítulo (e.g., "Persona A:"). Cuando se detecta un cambio de hablante, se envía una pregunta al LLM.

---

#### **4.3 Envío Manual de Preguntas al LLM**
Se implementará una combinación de teclas para enviar preguntas manualmente al LLM. Esto se manejará mediante un listener de eventos en el navegador externo.

##### **Código de Ejemplo para Combinación de Teclas**
```javascript
document.addEventListener('keydown', (event) => {
  if (event.ctrlKey && event.key === 'q') { // Ctrl + Q para enviar pregunta
    const question = prompt('Escribe tu pregunta para el LLM:');
    if (question) {
      socket.send(JSON.stringify({
        event: 'manual-question',
        question: question
      }));
    }
  }
...
function createWebSocketServer(mainWindow) {
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
      ...
      if (data.event === 'speaker-change') {
        console.log('Cambio de hablante:', data.speaker);
        mainWindow.webContents.send('speaker-change', data);
      } else if (data.event === 'manual-question') {
        console.log('Pregunta manual enviada:', data.question);
